...
class LaporanTransaksiOrderController extends Controller
{
	public function index(Request $request)
	{
		return view('pages.distributor.laporan.index');
	}
	...
	public function cetakExcel(Request $request)
	{
		...
		$transaksi = Pesanan::where('user_id', auth()->user()->id)
        ...
        $sheet->setCellValue('A2', 'REKAP TRANSAKSI ORDER');
        ...
        $i +=1;
        $sheet->mergeCells("C$i:F$i");
        $sheet->setCellValue('C'.$i, 'Distributor');
        $i +=3;
        $sheet->mergeCells("C$i:F$i");
        $sheet->setCellValue('C'.$i, auth()->user()->nama);

        $namafile = 'laporan-transaksi-order-'.bulanIndo($request->bulan).'-'.$request->tahun.'.xlsx';
