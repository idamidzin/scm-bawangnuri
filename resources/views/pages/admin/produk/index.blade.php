	...
	<div class="card">
		<div class="card-header">
			<div class="card-title">
				<a href="{{ route('produk.index') }}" class="{{ $is_trash ? 'text-success':'text-muted' }}"> Produk
					<span class="badge badge-pill badge-info">{{ $produk_count }}</span>
				</a>
				&nbsp; | &nbsp;
				<a href="{{ route('produk.index') }}?status=trash" class="{{ $is_trash ? 'text-muted':'text-danger' }}">
					Sampah
				...
				<table class="table table-bordered table-hover" id="datatable">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center">
								@if($row->foto)
								<a href="{{ asset('storage/produk/'.$row->foto) }}">
									<img src="{{ asset('storage/produk/'.$row->foto) }}" width="100px">
								</a>
								@else
								-
								@endif
...
<script type="text/javascript">
	...
	function execRemove(method, hashid) {
		$("#action-form").attr('action', 'produk/delete/' + hashid);
	...
	function restore (hashid) {
		$("#action-form").attr('action', 'produk/restore/' + hashid);
